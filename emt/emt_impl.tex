...
We ensure that EMT does not break performance characteristics, e.g., 
	...
	of instruction cache hit rates in most cases (\S\ref{sec:eval:eval-EMT-radix-vs-vanilla}). 
...
%	cache hit rates (\S\ref{sec:eval:eval-EMT-radix-vs-vanilla}).
...
% \schai{Our evaluation (\S\ref{sec:eval:eval-EMT-radix-vs-vanilla}) show that EMT has negligible overhead (within the standard deviation)}.
...
\caption{\bf Rewriting code in Figure~\ref{fig:need:linux-performance-opt} with EMT.}
\label{fig:interface-compare}
%\vspace{-5pt}
\end{figure}
...
\begin{figure}[H]
	...
	\label{fig:need:linux-performance-opt-new}
\end{figure}
\end{comment}
...
%	(e.g., Figure~\ref{fig:need:linux-no-interface});
%    we did not understand certain semantics which 
%    caused issues on ECPT (\S\ref{sec:ecpt_impl}).
...
% Figures~\ref{fig:impl:refactor} and~\ref{fig:impl:refactor} shows the refactored code of 
 %   the vanilla Linux code in Figure~\ref{fig:need:linux-no-interface} 
 %   and~\ref{fig:need:linux-performance-opt}, respectively.
...
For example, while we are working with the PF handler (Figure~\ref{code:PFhandler}) in linux kernel,
...
A generic example of the range operations is Figure \ref{code:walk-range}
...
Meanwhile, EMT-Linux reserves hardware-specific optimizations like in Linux---both % (Figure~\ref{fig:interface-compare-linux}).
...
Figure~\ref{fig:interface-compare} shows the code in
	EMT-Linux that implements the hardwired Linux routine in Figure~\ref{fig:need:linux-performance-opt}.
It uses the translation-object iterator (see Figure~\ref{code:iternext}) to scan all the pages in a memory region.
...
Figure~\ref{fig:pfh-example} shows a skeleton of the page fault handler in EMT-Linux.
...
%	despite one having been performed a few lines before for the same page (Figure~\ref{fig:interface-compare-pmap}). 
...
% To take the PFH as an example (Figure~\ref{fig:pfh-example}),
...
%	(details discussed in \S\ref{sec:reflection:sparse}).
...
%	One optimization is to seperate two page tables, and create a dedicated registers to enable switch between them.
	...
	% The basic functions of translation database (\ref{fig:api:db})
