...
%	architectures like ECPT in \S\ref{sec:ecpt_impl}.
Figure~\ref{fig:emt-overview} gives a high-level overview of EMT.
...
% As discussed in \S\ref{sec:need:hardwire},
...
% satisfy the needs (\optgrps in Figure \ref{figure:interface-internal}).
...
The interface eliminates architecture-specific or overloaded semantics. % (\S\ref{sec:need:hardwire}).
...
%	negligible overhead (\S\ref{sec:eval:eval-EMT-radix-vs-vanilla}).}{
...
%		in vanilla Linux.}
%		\label{figure:interface-overview}
...
Despite many different translation architectures (\S\ref{sec:background}),
...
%	routines hardwired 
%	to the \textit{shape} of translation data (\S\ref{sec:need}).
...
%	and recently proposed translation architectures (\S\ref{sec:design:generality}).
...
% to allow the system to function properly (Figure~\ref{fig:emt-overview}).
...
%		code to allow the system to function properly (Figure~\ref{fig:emt-overview}).
...
%		We show in \S\ref{sec:todo} that customizing some of them leads to 
...
Figure~\ref{fig:api} gives examples of basic function APIs.
...
The API design avoids 
	...
	(\S\ref{sec:background}) or page table entry schema.
It also prevents overloaded semantics like \texttt{\small pmd\_none\_or\_clear\_bad} (Figure~\ref{fig:need:linux-no-interface})
	due to software evolution.
% Linux's memory management routines can be refactored
%	and ported to EMT (\S\ref{sec:impl}).
...
% The first group manipulates the structure of the page table (\S\ref{sss:fg_1}). 
% The second group reads and writes the content of PTE (\S\ref{sss:fg_2}). 
% The last group controls the hardware configuration (\S\ref{sss:fg_3}).
...
A translation object encodes a virtual-to-physical address mapping 
	and all its associated metadata. %}{effective translation entry (\S\ref{sec:design:principle})}. %\jyk{Corresponds to the PTE of x86.}
...
It can also be implemented by multiple co-existing page tables 
	...
	and for a shared kernel space across processes; see \S\ref{sec:ecpt_impl}),
...
Its basic functions % are routines that change the translation structure,
	% as shown in Table~\ref{table:api}.
% These routines 
...
%	and a shared kernel space across processes (\S\ref{sec:ecpt_impl}).}
...
% Table \ref{table:pts-fg} lists the member functions of \ptsfg.
...
% Table \ref{table:pte-fg} shows the list of
...
Figure~\ref{fig:api-cus} gives examples of customizable function APIs.
...
Currently, EMT exposes different groups of customizable functions,
	as exemplified by Figure~\ref{fig:api-cus}.
...
The performance of such operations is critical, e.g., 
	...
	can reduce page fault handling cost by 52.5\% (\S\ref{ss:perf-ecpt}).
...
Figure~\ref{code:iternext-gen} shows the default implementation of the iterator's \texttt{\small tobj\_iter\_next()} 
...
Figure~\ref{code:iternext-x86} shows the customized implementation of the x86-64 MMU driver
	that leverages
	spatial locality of the radix tree to directly increment the pointer to get the next object.
We discuss the implementation of the ECPT MMU driver in \S\ref{ss:ecpt-mmu-driver}.
...
% ---the former checks one PMD entry while the latter checks multiple VMA attributes;
	...
	range operations like \texttt{\small aspace\_void()}, discussed in \S\ref{sec:reflection:sparse}
...
In contrast, ECPT has more expensive
	range operations as a scan in the hash table is required, discussed in \S\ref{sec:reflection:sparse}
	and thus the order of checks is opposite to x86-64's radix-tree-based design.
...
% For ECPT, software page table access is expensive even after an optimized iterator is implemented (\S\ref{ss:perf-ecpt}).
...
%	and a system setting check to see if THP is currently enabled (Figure \ref{code:thplinux}).
...
% To exploit such locality, the x86-64 MMU driver may add the deltas shown in Figure \ref{code:iterx86}
...
% The implementation of the generic and ECPT-specific version is shown in Figure \ref{code:thpgen}.
...
%	a large address range;
	ECPT can potentially enable locks on an entry cluster, a way, or a table (\S\ref{sec:multithreading}).
...
% However, in ECPT, this is not the case as some bits are always meaningful regardless of the P bit (Figure \ref{fig:ecpt-impl:ecpt-cluster}).
...
We describe how to instantiate tree-based memory translation
	using radix tree as an example (see~\S\ref{xxx})
	and hashing-based memory translation using ECPT~\cite{skarlatos2020ecpt}
	as an example (see~\S\ref{xxx}).
