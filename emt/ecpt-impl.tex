...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.495\textwidth]{figs/emu.pdf}
	\caption{\bf Emulator toolchain for OS development and evaluation 
		on experimental memory architectures.}
%	\vspace{-2.5pt}
	\label{fig:eval:emulator}
\end{figure}

We develop an emulator-based toolchain using QEMU~\cite{plat:qemu} (Figure~\ref{fig:eval:emulator}).
...
% We implemented the ECPT MMU and ECPT MMU driver together (\S\ref{sec:lessons}).
...
The instruction trace helps analyze kernel and userspace behavior
	(\S\ref{ss:eval:eval-ecpt-os}), as well as 
	hardware simulation.
...
The driver also manages nine registers
	for the kernel page tables (\S\ref{ss:ecpt-kernelspace}).
...
to improve locality. 
To do so, we construct the VPN tag (Figure~\ref{fig:background:ecpt-native})
	with bits from multiple translation entries, as shown in Figure~\ref{fig:ecpt-impl:ecpt-cluster}. 
...
PUD and PMD entries use the same design but with
	24 bits and 15 bits for the VPN tag. % (Figure~\ref{fig:background:ecpt-native}).}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.475\textwidth]{figs/pte-cluster.pdf}
...
%	\vspace{-5pt}
	\label{fig:ecpt-impl:ecpt-cluster}
\end{figure}
...
\para{Optimizations.} We implemented a series of optimizations through the customizable functions,
	driven by benchmarking and profiling (\S\ref{ss:perf-ecpt}).
For example, we customize the iterator % \texttt{\small tentry\_iter\_next()} 
	% function %(Figure~\ref{code:iternext}) 
...
The idea is to exploit locality within a translation entry cluster:
	...
	%We show that this optimization leads to a major reduction of OS overhead (\S\ref{ss:perf-ecpt}).
One common optimization pattern is to minimize the cost of range operations---the  
	default implementation (e.g., Figure~\ref{code:iternext-gen}) often takes too many fine-grained hashing-based operations.
...
In fact, if one is considering rebuilding kernel for a new translation architecture,
	I would recommend starting from the user page table setup for example in \S \ref{sss:ecpt-init}.
...
%	(\S\ref{ss:ecpt-kernelspace}) such as the paradox involved in modifying translations 
...
This creates a challenge for ECPT which needs to move translation entries 
	...
	moving window, if the entries map the kECPT itself 
		...
		as shown in Figure~\ref{fig:ecpt-impl:paradox}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\textwidth]{figs/paradox.pdf}
	\caption{{\bf An example of self-reference paradox caused by the need of 
		...
		for moving kECPT entries (\S\ref{ss:ecpt-self-reference}).}}
%	\vspace{-7.5pt}
	\label{fig:ecpt-impl:paradox}
\end{figure}
...
% In practice, ECPT exhibits much higher memory usage than what is documented in the original paper even with all optimizations applied (\S\ref{sec:ecpt-opti}), as shown in Table \ref{}\JZtodo.
