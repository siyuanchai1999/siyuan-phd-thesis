...
%	to record the mapping from virtual to physical addresses,
	as depicted in Figure~\ref{emt:fig:background:radix-native}.
...
%	\vspace{-15pt}
	\centering
	\includegraphics[width=0.9\textwidth]{figs/x86-walk}
	\caption{\bf Radix-tree based page table walk in x86-64 ISA.}
	\label{emt:fig:background:radix-native}
\end{figure}
...
% Figure \ref{emt:figure:background:radix-native} illustrates the process.
...
Specifically, FPT tries to merge L4 and L3, as well as L2 and L1, in Figure~\ref{emt:fig:background:radix-native},
	shortening the walk by half.

\para{Hashing-based Translation and ECPT.}
%\label{emt:sec:background:ecpt}
...
%	reduce translation overhead by an average of 41\% over x86-64. %'s radix-tree scheme. % (\S\ref{emt:sec:background:tree})
...
Figure~\ref{emt:fig:background:ecpt-native} depicts ECPT-based memory translation.
...
% Figure \ref{emt:figure:background:ecpt-native} illustrates the page walk of ECPT.
