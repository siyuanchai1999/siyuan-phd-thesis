...
\begin{figure}[H]
	\centering
\begin{minted}[xleftmargin=17.5pt,linenos,breaklines,fontsize=\scriptsize,escapeinside=@@]{C}
...
\label{fig:need:linux-no-interface}
\end{figure}
...
Figure~\ref{fig:need:linux-no-interface} shows a representative example in Linux, where a memory
...
In Figure~\ref{fig:need:linux-no-interface}, \texttt{\small pmd\_none\_or\_clear\_bad()} 
...
%	the lack of extensibility has become a barrier to hardware innovations as discussed in \S\ref{sec:intro}.
...
\label{fig:need:linux-performance-opt}
\end{figure}
...
Figure~\ref{fig:need:linux-performance-opt} shows three common patterns of optimizations in existing Linux code:
...
%	whereas, under the VFS interface (Figure \ref{code:vfs}), there is only a single (\texttt{file}-based) \texttt{read}/\texttt{write} interface.
