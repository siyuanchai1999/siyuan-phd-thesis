...
We collect the instruction traces and memory traces by running the macro benchmarks
    ...
    and use the traces for hardware simulation, as shown in Figure~\ref{fig:eval:emulator}.
...
For the ECPT system, despite that
	ECPT increases IPC, 
    our ECPT MMU driver introduces more kernel instructions than the x86-64 MMU driver (\S\ref{ss:eval:eval-ecpt-os}).
Therefore, we include kernel overhead (mostly page fault handling as shown in Figure~\ref{fig:eval:eval-kern-inst-breakdown}) 
	and measure the total cycles for running 
	macro benchmarks and applications.
The result is shown in Figure~\ref{fig:eval:eval:e2e-4KB}.
...
If we only consider the running phase, then the ECPT system 
    ...
    x86-64 system, as shown in Figure~\ref{fig:eval:eval:e2ert-4KB}.
...
%	the results are in Figure~\ref{fig:eval:eval:e2ert-4KB}.
...
We discussed optimizations that are implemented using the 
    ...
    both in the x86-64 MMU driver (Figure~\ref{code:iternext})
    and in the ECPT MMU driver (\S\ref{ss:ecpt-mmu-driver}).
...
Figure~\ref{code:thpgen} shows the implementaions of \texttt{\small thp\_eligible}
...
In contrast, ECPT has more expensive range operations like \texttt{\small addr\_range\_void()}
    (see the discussion in \S\ref{sec:ecpt:os:perf});
...
Figure~\ref{fig:eval:appendix} shows the effectiveness of this performance 
    optimization in the ECPT system,
    together with the iterator optimization 
    (which is discussed in Figure~\ref{fig:eval:eval-ecpt-flame-thp}).
% ~\ref{fig:eval:eval:opt-group-effect-4k} and~\ref{fig:eval:eval:opt-group-effect-thp}
%	compare the effect of the iterator
%	optimization with the huge page optimization (\texttt{thp\_eligible}) in Figure~\ref{fig:api-cus}.
...
\begin{subfigure}[b]{0.24\textwidth}
	...
	\vspace{-18pt}
	\caption{4KB}
	\label{fig:eval:eval:opt-group-effect-4k}
\end{subfigure}%
	% Subfigure 2
	% \vspace{-2cm}
	% \vspace{0.1cm}
...
\begin{subfigure}[b]{0.24\textwidth}
	...
	\vspace{-18pt}
	\caption{THP}
	\label{fig:eval:eval:opt-group-effect-thp}
\end{subfigure}
	\vspace{-20pt}
...
Figures~\ref{fig:eval:eval-radix-never-vs-ecpt-never-flame-a} and~\ref{fig:eval:eval-radix-never-vs-ecpt-never-flame-b} 
...
\begin{subfigure}[b]{0.49\textwidth}
	...
	\label{fig:eval:eval-radix-never-vs-ecpt-never-flame-a}
\end{subfigure}%
...
\begin{subfigure}[b]{0.49\textwidth}
	...
	\vspace{-15pt}
	\caption{ECPT MMU driver}
	\label{fig:eval:eval-radix-never-vs-ecpt-never-flame-b}
\end{subfigure}
	\vspace{-20pt}
