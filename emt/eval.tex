...
We run EMT-Linux with the ECPT MMU driver on top of an emulated ECPT MMU 
	using our emulation framework (\S\ref{sec:eval:emulator}).
...
%To address this dilemma, we assemble an emulation framework (\S\ref{sec:eval:emulator})
...
Table~\ref{table:workloads} shows the workloads.
...
\label{sec:eval:eval-EMT-radix-vs-vanilla}
...
Figure~\ref{fig:eval:microbenchmark} shows the results of \schai{kernel micro benchmarks} LEBench~\cite{bench:lebench}.
...
% 	%\vspace{-15pt}
% 	\label{figure:eval:eval-stack-lebench}
% \end{figure*}
Figure~\ref{fig:eval:macrobenchmark} shows that
	the overhead of EMT-Linux is less than 0.1\% on the macro benchmarks.
Figure~\ref{fig:eval:eval-radix-real-app} shows the normalized throughput,
	average latency, and tail latency of three real-world applications (with workloads in Table~\ref{table:workloads}) on 
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.14\textwidth}
		...
		\caption{Throughput}
		\label{figure:eval:eval-radix-real-app-throughput}
	\end{subfigure}%
	% 
	...
	\begin{subfigure}[b]{0.14\textwidth}
		...
		\caption{Avg. Latency}
		\label{figure:eval:eval-radix-real-app-avg-latency}
	\end{subfigure}%
	% 
	\hspace{0.01\textwidth}%
	% Subfigure 3
	\begin{subfigure}[b]{0.14\textwidth}
		...
		\caption{P99 Latency}
		\label{figure:eval:eval-radix-real-app-p99-latency}
	\end{subfigure}
	\caption{\bf Performance overhead of EMT-Linux over vanilla Linux
		on real-world applications}
	\label{fig:eval:eval-radix-real-app}
\end{figure}
...
% (more information can be found in Appendix~\ref{appendix:perf})}.
% \schai{Instruction cache hit rates on EMT-Linux differ by at most 0.8\% from those on vanilla Linux (see Appendix \ref{appendix:perf})}.
...
%	\schai{\texttt{addr\_range\_void} (Figure ~\ref{code:thpgen})
...
%	the major kernel activities are in 4K anonymous page fault handler (as shown in Figure~\ref{fig:eval:eval-kern-inst-breakdown}).
%	EMT-Linux's performance difference is only of 0.5\% according to measurements from microbenchmarks in Figure ~\ref{fig:eval:microbenchmark}.
...
% kernel operations (Figure ~\ref{figure:eval:eval-stack-lebench}). The correct
...
\begin{subfigure}[b]{0.23\textwidth}
	...
	\caption{4KB}
	\label{figure:eval:eval-kern-inst-breakdown-4k}
\end{subfigure}%
\hspace{0.01\textwidth}
\begin{subfigure}[b]{0.23\textwidth}
	\centering
...
% Table~\ref{table:configuration} shows the hardware configuration we simluated,
...
% 	%\vspace{-15pt}
% 	\label{figure:eval:eval-kern-inst-breakdown}
% \end{figure}
...
% 	% \vspace{-0.1cm}
% 	\caption{ 4KB}
% 	\label{fig:eval:eval-kern-inst-breakdown-4KB}
% \end{subfigure}%
...
For Radix, these checks are cheap (0.39\% of total kernel work)
	because a PMD entry has information on PTE tables. % (Figure~\ref{fig:background:radix-native}).
...
We discussed the potential solution % using special entries in PMD-ECPT 
	in \S\ref{sec:reflection:sparse}.
...
\begin{table}
	...
	\label{table:Kernel-inst-percentage}
\end{table}
...
% Figure~\ref{figure:eval:eval-kern-inst-breakdown-4k} and Figure~\ref{figure:eval:eval-kern-inst-breakdown-thp} 
...
% What are the observations from Figure~\ref{figure:eval:eval-kern-inst-breakdown}
% 	and what are the conclusions?
% Also, please improve Figure 13 as we discussed yesterday.} 

% \schaic{The plot ~\ref{figure:eval:eval-kern-inst-breakdown} above needs to be modified in format, recapture loading phase data, as well as add gups and sysbench data}.
...
%	\label{fig:eval:eval-radix-always-vs-ecpt-always-flame-a}
\begin{subfigure}[b]{0.34\textwidth}
	\centering
...
%	\end{subfigure}%
	...
	% \label{fig:eval:eval-radix-always-vs-ecpt-always-flame-b}
	% \end{subfigure}
	\begin{subfigure}[b]{0.49\textwidth}
	\centering
	...
	\label{fig:eval:eval-ecpt-flame-thp}
\end{figure}
...
Figure~\ref{fig:eval:eval-ecpt-flame-thp} uses flame graphs 
	to show the effectiveness of iterator optimization running 
	GraphBIG BFS with THP. % (we discuss another customizable function optimization in Appendix~\ref{sec:app:another}).
With the iterator optimization (\S\ref{ss:ecpt-mmu-driver}), 
	ECPT can save 
	% 47.27\%
	49.0\% 
...
The optimization drastically saves the work of hashing and lookups of the architecture-neutral default implementation,
	by incrementing the pointer to an entry directly, as long as the entry is within an entry cluster (Figure~\ref{fig:ecpt-impl:ecpt-cluster}).
...
%	by exploiting PTE clustering and compaction design (Figure~\ref{fig:ecpt-impl:ecpt-cluster}).
...
% 	\label{figure:eval:opt-group-effect}
% 	\vspace{-15pt}
% \end{wrapfigure}
...
% 	can further reduce the execution count to 31.61\% of the baseline. 
	% (Figure \ref{figure:eval:opt-group-effect}a).
% With THP, the EMT-exposed optimizations reduce the kernel overhead by \jztodo{100}\% and then \jztodo{100}\%, respectively (Figure \ref{figure:eval:opt-group-effect}b).

% code:iterx86
The page fault handler checks on \texttt{thp\_eligible}
	shown in Code snippet \ref{code:thpgen} before it proceeds to build a huge page.
...
For ECPT, 1) is much cheaper than 2) since 2) involves iterating through the address range
	as we mentioned in Section \ref{ss:perf-ecpt}.
...
% 		by exploiting the spatial contiguity of translation entries within a PTE cluster (\S\ref{sss:ecpt-cluster})
% 			and thus avoiding redundant hash computations.
% Further, as shown in Figure \ref{code:thpgen}, the ECPT MMU driver can optimize the eligibility checker
...
Figure~\ref{fig:eval:eval:pw-4KB} and ~\ref{fig:eval:eval:pw-thp} shows the speedup of page table walk latency
	of a ECPT MMU, normalized to a x86-64 MMU, 
	using the hardware configuration in Table~\ref{table:configuration}.
...
\schai{x86-64 radix and ECPT MMU drivers} using the DynamoRIO simulator~\cite{plat:dynamorio}.
	% using DynamoRIO~\cite{plat:dynamorio}. % (Figure~\ref{fig:eval:emulator}).
...
We use the macro benchmarks and the three application workloads described in \S\ref{sec:eval:metho}.
% \jz{
...
% % 	and ran hardware simulation using DynamoRIO on the trace(Figure~\ref{fig:eval:emulator}). 
% % We run hardware simulation using DynamoRIO with 
% % 	instruction and memory traces collected by our toolchain (Figure~\ref{fig:eval:emulator}).
...
% We reuse the applications we evaluated in previous sections (see \S\ref{sec:eval:metho}) for consistency,
...
% Figure~\ref{fig:eval:eval:pw-4KB} and \ref{fig:eval:eval:ipc-4KB}
%	shows the speedup of page walk latency and IPC of \eecpt over \eradix.
\schai{As shown in Figures~\ref{fig:eval:eval:pw-4KB} and~\ref{fig:eval:eval:ipc-4KB},
...
We find that IPC, which measures instruction throughput, does not fully 
	...
	more kernel instructions than the x86-64 MMU driver (\S\ref{ss:eval:eval-ecpt-os}).
Hence, we measure the total cycles for running 
	macro benchmarks and applications,
	including the OS overhead (mostly page-fault handling as shown in Figure~\ref{fig:eval:eval-kern-inst-breakdown}).
\schai{Figure~\ref{fig:eval:eval:e2e-4KB} shows that},
...
% \schaix{Detailed results are in Appendix~\ref{sec:details_hwsim}}.
...
As shown in \S\ref{ss:perf-ecpt}, memory translation architectures can have profound implications for OS performance. 
...
With only the running phase considered, ECPT 
	...
	compared to the x86-64 system (Figure~\ref{fig:eval:eval:e2ert-4KB}).
...
% We time the software overhead (\S\ref{sec:eval:eval-EMT-radix-vs-vanilla} and \S\ref{ss:perf-ecpt})
%	and the IPC (Figure~\ref{fig:eval:eval:ipc-4KB}) to extrapolate the application runtime.}

% \jz{Figure~\ref{fig:eval:eval:e2e-4KB} show the extrapolated end-to-end application runtime of \eecpt over \eradix.
...
% As shown in Figure~\ref{figure:eval:eval-kern-inst-breakdown-4k}, ECPT shows a 1.56x increase in kernel work
%	compared to radix.
...
\schai{We start with a FPT configuration 
	that flattens L3+L2 tables (see Figure~\ref{fig:background:radix-native})
	which was implemented by the original OS prototype~\cite{park2022fpt}.
% We evaluated this configuration following the methodology in \S\ref{sec:eval:hwsim}. 
...
As shown in Figures~\ref{fig:eval:eval:fpt-pw-4KB}, ~\ref{fig:eval:eval:fpt-ipc-4KB},
	~\ref{fig:eval:eval:fpt-e2e-4KB},
...
With this configuration, compared with Linux on x86-64,
	...
	and total cycles by 3.7\% (Figures~\ref{fig:eval:eval:fpt-pw-4KB},~\ref{fig:eval:eval:fpt-ipc-4KB}, and~\ref{fig:eval:eval:fpt-e2e-4KB}).
% OS overhead by 3.4\%
...
Flattening both L4+L3 and L2+L1 in FPT yields performance comparable to ECPT in the running phase.
	% FPT reduces total cycles by 6.4\% and ECPT by 6.6\% (Figure ~\ref{fig:eval:eval:fpt-e2ert-4KB} and~\ref{fig:eval:eval:e2ert-4KB}).
Compared with radix, ECPT and FPT 
	reduce total cycles by 6.6\% and 6.4\%, respectively 
	(Figures~\ref{fig:eval:eval:e2ert-4KB} and~\ref{fig:eval:eval:fpt-e2ert-4KB}).
