...
We use page fault handler
  ...
  contributes to the most kernel activities in our evaluation (Figure~\ref{fig:eval:eval-kern-inst-breakdown}).
Figure~\ref{code:pfh-ve} shows the code snippets of page fault handlers 
  in vanilla Linux 
  and EMT-Linux. 
...
%	(see Figure~\ref{fig:tobj-x86} for an x86 example),
...
%	which generated the most kernel activities in our experiments (Figure \ref{fig:eval:eval-kern-inst-breakdown}),
...
Figure~\ref{fig:eval:asm} shows 
  the x86 assembly generated by compiling the code snippets in Figure~\ref{code:pfh-ve}.
...
% Figure~\ref{fig:eval:x86-asm} and \ref{fig:eval:emt-asm} shows the compiled code in x86 \jzx{ASM}{assembly}
...
We can see in Figure~\ref{fig:eval:asm} that
...
We count the instructions of the page fault handlers
	in the real EMT-Linux and vanilla Linux artifacts ({\it not} simplified ones as shown in Figure~\ref{code:pfh-ve}).
