...
We continuously test and evaluate our system, not only 
    for correctness (\S\ref{ss:correctness}) but also for performance.
% For example, % when refactoring Linux to EMT-Linux (\S\ref{sec:impl}),
...
We developed the instruction analysis 
    with flame graphs~\cite{tool:flamegraph} in our emulator toolchain. % (\S\ref{sec:eval:emulator}) 
