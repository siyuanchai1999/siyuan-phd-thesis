...
On allocation through \texttt{cudaMallocManaged},
    pages are owned by the runtime
    and migrated on demand between CPU and GPU
...
On modern hardware with coherent CPU--GPU interconnects
    (e.g., NVLink-C2C on GH200),
    these properties become even more attractive
    because the marginal cost of CPU-resident accesses drops sharply.
...
When the working set fits in GPU memory (\figurename~\ref{uvm:fig:motivation}a),
    ...
    is highly efficient.
In contrast, naive UVM without caching (U0) is $53\times$--$69\times$ slower than C1,
    and even application-level KV-cache offloading (A) is $31\times$--$42\times$ slower
...
\begin{figure}[H]
    ...
    \caption{Performance gap of UVM for Hugging~Face OPT-13B inference on H100 PCIe.
        ...
        naive UVM (U0) is over $50\times$ slower than vanilla PyTorch (C1).
        (b)~When the GPU is oversubscribed (batch sizes $32$--$64$),
        vanilla PyTorch crashes with OOM,
        and naive UVM thrashes, running up to $5\times$ slower than application-level KV-cache offloading (A).}
...
Modern AI frameworks such as PyTorch
    ...
    which assume that allocated memory is strictly device-resident.
...
This caching policy is critical for performance
    ...
    {\uvmname}'s discard-based eviction.
