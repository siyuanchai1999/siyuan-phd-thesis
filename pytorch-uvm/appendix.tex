\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \label{uvm:fig:stress-flush}
    \end{subfigure} 
    \caption{Rx stress test (multiple flows per core): \uvmname{} consistently matches vIOMMU-off throughput.
    }
    \label{uvm:fig:stress}
\end{figure}
...
%     \label{uvm:fig:tx-dlf}
% \end{figure}

\subsection{Threat Model}
\label{uvm:sec:threat_model}
...
{\uvmname} follows the threat model of strict protection mode of Linux IOMMU~\cite{nadav2010iommu} and prior work~\cite{markuze2018damn,Amit2011vIOMMU,RajapakshaIOMMU}.
...
{\uvmname} defends against attacks that control malicious or compromised IO devices.
...
\subsection{Memory Overhead of \uvmname}

% \textbf{Parameter sensitivity of async invalidation.}

% Figure \ref{uvm:fig:sensitivity} shows the parameter sensitivity of async invalidation implemented in distributed-leader-follower scheme.
...
% Intuitively, to achieve the same throughput, more pages will be activated by {\uvmname}. 
We show that the memory overhead of \brand
for our evaluation setup is pretty small.
We run 24 Iperf flows for 120 seconds, sampling the number of active pages every 0.5 seconds for {\uvmname} and vIOMMU-off.
\uvmname's throughput is within 1\% of vIOMMU-off in these experiments.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\columnwidth}
        ...
        \label{uvm:fig:tx_exp_memory_tx}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\columnwidth}
        ...
        \label{uvm:fig:tx_exp_memory_total}
    \end{subfigure}
    \caption{For Tx-server setup, {\uvmname} incurs less than 3.3\%
     memory overhead over vIOMMU-off.}
    \label{uvm:fig:tx_memory}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\columnwidth}
        ...
        \Description{Plot showing TX page usage over time.}
        \caption{Active pages required by the Tx datapath over time.}
        \label{uvm:fig:rx_exp_memory_tx}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\columnwidth}
        \includegraphics[width=\columnwidth]{figures/rx_mem/mem_fig_tx_rx_active_pages.pdf}
        \Description{Plot showing RX+TX page usage over time.}
        \caption{Active pages required by the Rx and Tx datapath over time.}
        \label{uvm:fig:rx_exp_memory_total}
    \end{subfigure}
    \caption{For Rx-server setup, {\uvmname} incurs less than 2.3\% additional memory overhead over vIOMMU-off.}
    \label{uvm:fig:rx_memory}
\end{figure}

\para{Tx-server.} We measure active physical pages for Tx held by the mlx5 driver. The Tx datapath shows an increase by 30.5\%, an inherent cost of deferring free page (Figure~\ref{uvm:fig:tx_exp_memory_tx}).
However, Figure~\ref{uvm:fig:tx_exp_memory_total} shows that when accounting for both Tx and Rx-datapath active pages, the total page count increases by only 3.28\% over vIOMMU-off,
...
As shown in Figure~\ref{uvm:fig:rx_exp_memory_total}
\uvmname{} requires only 2.23\% more total pages over vIOMMU-off.
The active page count for Tx datapath only (Figure~\ref{uvm:fig:rx_exp_memory_tx}) shows a larger relative difference,
