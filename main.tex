...
\abstract{
Modern computing bears little resemblance to that of a decade ago: datacenters
are now heterogeneous fabrics of CPUs, GPUs, accelerators, and high-bandwidth
interconnects, running workloads---LLM training, in-memory analytics,
network intensive applications---that push the hardware to its limits.
Memory is the central resource of this stack; every workload, on every device,
ultimately reads and writes memory.
Yet the system software layers that manage it---the OS kernel, the hypervisor,
device drivers, and AI frameworks---were conceived for a world that no longer
exists, and the cost of this mismatch is paid in performance, security, and
extensibility.
...
\emph{PyTorch-UVM} integrates a UVM-aware caching allocator and discard-based
eviction into PyTorch, enabling practical GPU memory oversubscription for LLM
inference without modifying application code.

Together, these systems demonstrate that bottlenecks in modern memory
management arise not from hardware limitations but from system software
designed without awareness of the hardware and workloads it now serves, and
that targeted changes to those software layers suffice to eliminate them.
}
